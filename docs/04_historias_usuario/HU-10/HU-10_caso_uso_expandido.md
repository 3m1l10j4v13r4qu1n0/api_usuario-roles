# HU-10: Caso de Uso Expandido (Quitar rol)

**Actor Principal:** Administrador (rol `ADMIN`).
**Precondición:** El administrador posee un token JWT válido.

## Flujo Principal (Éxito)
1. El administrador envía `DELETE /usuarios/{usuario_id}/roles/{rol_id}`.
...
4. El repositorio quita el rol al usuario; si el usuario no existe → `UsuarioNoEncontradoError` → 404.
...
## Flujo Alternativo 4 — Revocación en caliente
- Tras el paso 5, el próximo request del usuario destino resuelve su estado real sin el rol
  (sin re-login) — USR-RN11/RN12.

## Postcondición
- El usuario dejó de tener el rol y la autorización lo refleja sin re-login.
